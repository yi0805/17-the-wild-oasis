...
- After a successful update only, strict canonical ownership is checked and a fresh exact-URL cabin reference query prevents removal of shared images. Every destructive Storage removal performs its own final fresh live-reference check immediately before deletion; reference-query failures fail closed and preserve the successful primary update.
...
- Added a bounded batch-of-10 retry service. Every queued item is reconstructed, revalidated as canonical, and receives the same final reference check immediately before removal; failures are isolated per item.
...
- Focused cleanup, mutation, and retry-trigger regressions: 4 files / 26 tests passed.
- `npm test -- --reporter=dot`: passed, 33 files / 168 tests.
- `npm run build`: passed. Existing large-chunk warning remains (`991.79 kB` minified / `284.19 kB` gzip).
