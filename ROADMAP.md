...
- 2026-09-10 Task 025 repository implementation: hardened `updateCurrentUser` avatar replacement ordering so an authenticated-user lookup precedes upload, an Auth metadata failure rolls back only the new object, and a prior avatar is removed only after success when it is an exact same-user application-owned object. Ten local service tests bring the suite to 12 files / 72 tests. The forward-only avatar SELECT/DELETE policy migration is proposed in the repository only; hosted application and verification remain pending human review and must not be represented as complete.
