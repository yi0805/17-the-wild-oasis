...
- 2026-09-06 Phase 2.4b: migrated the active CabinRow, CabinTable, and CabinTableOperations boundaries to TSX. The generated nullable `cabins` row remains the source of truth: display fallbacks avoid null coercion, non-empty images render while missing images use a local placeholder, and duplication requires a runtime-verified complete row. Shared filter/sort options now runtime-narrow URL values with `all` and `name-asc` fallbacks. Cabin sorting explicitly keeps null values last for either direction and sorts a copy rather than mutating TanStack Query data. Three JSX boundary tests cover nullable display, malformed sorting, and null-last non-mutating sorting; 5 test files / 23 tests passed locally. No generic UI, service, or hook runtime logic changed; final GitHub Actions verification is recorded in the Task 017 handoff.
