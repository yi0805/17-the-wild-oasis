...
const { getStaysTodayActivity, updateBooking } = vi.hoisted(() => ({ getStaysTodayActivity: vi.fn(), updateBooking: vi.fn() }));
vi.mock("../../services/apiBookings", () => ({ getStaysTodayActivity, updateBooking }));
import TodayActivity from "./TodayActivity";
import { renderWithProviders } from "../../test/renderWithProviders";

function createActivity(overrides = {}) {
  return { id: 1, status: "unconfirmed", numNights: 1, guests: { fullName: "Taylor Guest", nationality: "New Zealander", countryFlag: "https://example.com/nz.svg" }, ...overrides };
}
function renderActivity(activities) {
  getStaysTodayActivity.mockResolvedValue(activities);
...
describe("TodayActivity", () => {
  ...
  it("renders arriving guest data and the check-in link", async () => {
    ...
    expect(screen.getByRole("img", { name: "Flag of New Zealander" })).toBeVisible();
    expect(screen.getByRole("link", { name: "Check in" })).toHaveAttribute("href", "/checkin/1");
  });
  it("checks out departing activities with the existing mutation payload", async () => {
    ...
    await waitFor(() => expect(updateBooking).toHaveBeenCalledWith(1, { status: "checked-out" }));
  });
  it.each([null, undefined, []])("renders no activity for %s data", async (activities) => {
